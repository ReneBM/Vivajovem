...
export default function Lideres() {
  ...
  async function confirmDelete() {
    ...
    try {
      const { error } = await supabase.from('lideres').delete().eq('id', liderToDelete);
      if (error) throw error;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-display font-bold text-foreground">Líderes</h1>
          <p className="text-muted-foreground mt-1">
            Gerencie os líderes do ministério
          </p>
        </div>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-4">
                    <Avatar className="h-14 w-14">
                      <AvatarImage src={lider.foto_url || undefined} alt={lider.nome} />
                    ...
                    <div>
                      ...
                      <Badge
                        ...
                        className={lider.status === 'ATIVO' ? 'bg-success/10 text-success hover:bg-success/20 mt-1' : 'mt-1'}
                      >
      ...
      <AlertDialog open={!!liderToDelete} onOpenChange={(open) => !open && setLiderToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="font-display">Tem certeza absoluta?</AlertDialogTitle>
            <AlertDialogDescription>
              Essa ação não pode ser desfeita. Isso excluirá permanentemente o cadastro do líder do sistema.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
