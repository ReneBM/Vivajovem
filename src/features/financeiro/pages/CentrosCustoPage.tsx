...
interface CentroCusto {
    id: string; nome: string; descricao: string | null; ativo: boolean; created_at: string;
}
...
export default function CentrosCustoPage() {
    ...
    async function fetchItems() {
        try {
            const { data, error } = await supabase.from('fin_centros_custo').select('*').order('nome');
            if (error) throw error;
            setItems((data as CentroCusto[]) || []);
    ...
    function resetForm() { setEditingId(null); setFormData({ nome: '', descricao: '', ativo: true }); }

    async function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        setIsSubmitting(true);
        try {
            const payload = { nome: formData.nome, descricao: formData.descricao || null, ativo: formData.ativo };
            if (editingId) {
                const { error } = await supabase.from('fin_centros_custo').update(payload as any).eq('id', editingId);
                if (error) throw error;
                toast.success('Atualizado!');
            } else {
                const { error } = await supabase.from('fin_centros_custo').insert(payload as any);
                if (error) throw error;
    ...
    async function handleDelete() {
        if (!deleteId) return;
        try {
            const { error } = await supabase.from('fin_centros_custo').delete().eq('id', deleteId);
            if (error) throw error;
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-display font-bold text-foreground">Centros de Custo</h1>
                    <p className="text-muted-foreground mt-1">Gerencie os centros de custo</p>
                </div>
                <Dialog open={isDialogOpen} onOpenChange={open => { setIsDialogOpen(open); if (!open) resetForm(); }}>
                    <DialogTrigger asChild><Button variant="hero"><Plus className="w-4 h-4" /> Novo Centro</Button></DialogTrigger>
                    <DialogContent className="sm:max-w-[400px]">
                        <DialogHeader>
                            <DialogTitle className="font-display">{editingId ? 'Editar' : 'Novo'} Centro de Custo</DialogTitle>
                            <DialogDescription>Preencha os dados</DialogDescription>
                        </DialogHeader>
                        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
                            <div className="space-y-2"><Label>Nome *</Label><Input value={formData.nome} onChange={e => setFormData({ ...formData, nome: e.target.value })} placeholder="Ex: Departamento de Jovens..." required /></div>
            ...
            <Card className="glass-card overflow-hidden">
                {loading ? <div className="flex items-center justify-center py-12"><Loader2 className="w-8 h-8 animate-spin text-primary" /></div>
                    : items.length === 0 ? <div className="flex flex-col items-center justify-center py-12 text-center"><Target className="w-12 h-12 text-muted-foreground/50 mb-4" /><h3 className="font-semibold">Nenhum centro cadastrado</h3><p className="text-sm text-muted-foreground mt-1">Comece cadastrando o primeiro</p></div>
                        : (
                            <Table>
                                <TableHeader><TableRow><TableHead>Nome</TableHead><TableHead className="hidden md:table-cell">Descrição</TableHead><TableHead>Status</TableHead><TableHead className="w-[100px]"></TableHead></TableRow></TableHeader>
                                <TableBody>
                                    {items.map(item => (
